...
if(isset($_POST['limit'])){
    ...
    //$sqlString = "SELECT DISTINCT * FROM cars, UserClick WHERE  cars.identity = UserClick.carid AND UserClick.clickdate>=DATE_SUB(now(),INTERVAL 2 DAY) GR ORDER BY UserClick.clickdate desc LIMIT $limit";        
	$sqlString = "SELECT * from cars WHERE identity in 
(select distinct identity from (
SELECT identity,clickdate FROM 
 cars, UserClick WHERE  cars.identity = UserClick.carid AND UserClick.clickdate>=DATE_SUB(now(),INTERVAL 2 DAY) ORDER BY  UserClick.clickdate desc) as A) LIMIT $limit";
